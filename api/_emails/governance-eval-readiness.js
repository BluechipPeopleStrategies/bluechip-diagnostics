...
function signalRich({ name, bandLabel, total, detail, ctaUrl }) {
  ...
  return {
    ...
    html: layout(`
      <p>Hi ${name},</p>
      <p>Quick note: your Governance Health Check result came in at ${bandLabel} (${total}/100).${detailSentence} That is a common read; most boards inherit their evaluation process rather than design it.</p>
...
function strongFoundation({ name, bandLabel, total, ctaUrl }) {
  ...
  return {
    ...
    html: layout(`
      <p>Hi ${name},</p>
      <p>Quick note: your Governance Health Check result came in at ${bandLabel} (${total}/100). That is a stronger evaluation practice than many boards ever build.</p>
