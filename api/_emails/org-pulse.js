...
const STRIPE_CLARITY_CALL_URL = 'https://buy.stripe.com/eVqaEW4Opettfab2SFco00d';

const SIGNAL_RICH_BANDS = new Set(['Pressure building', 'Mixed signal']);

export function buildOrgPulseEmail({ firstName, bandLabel, total }) {
  const name = firstName || 'there';
  const tier = SIGNAL_RICH_BANDS.has(bandLabel) ? 'signal-rich' : 'strong-foundation';
  return tier === 'signal-rich'
    ? signalRich({ name, bandLabel, total })
    : strongFoundation({ name, bandLabel, total });
}

function signalRich({ name, bandLabel, total }) {
  return {
    ...
    html: layout(`
      ...
      <p><strong>What we'd usually do next:</strong> a 30-minute Clarity Call to walk through your result, ask the few questions the diagnostic can't, and map out what a development path could look like for your organization specifically.</p>
      ${ctaButton('Book a Clarity Call — $99, 30 minutes')}
...
function strongFoundation({ name, bandLabel, total }) {
  return {
    ...
    html: layout(`
      ...
      <p>If there's a leadership transition, restructure, or major initiative on your horizon in the next two quarters, that's exactly the kind of moment a strong foundation pays for itself — and exactly the kind of moment a Clarity Call is most useful.</p>
      ${ctaButton('Book a Clarity Call — $99, 30 minutes')}
      <p>— Thomas<br/>BlueChip People Strategies</p>
    `),
  };
...
function ctaButton(label) {
  return `<p style="text-align:center;margin:28px 0;"><a href="${STRIPE_CLARITY_CALL_URL}" style="display:inline-block;background:#1a1a1a;color:#ffffff;text-decoration:none;padding:14px 24px;border-radius:4px;font-weight:600;">${label} →</a></p>`;
}
